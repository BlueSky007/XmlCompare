...
namespace DifferenceEngine
{
    ...
    {
        private const int BAD_INDEX = -1;//常量索引为-1；
        ...
        //构造函数，调用初始化方法：SetToUnkown
        public DiffState()
        {
            SetToUnkown();
        }

        //构造函数，初始化匹配状态为UnKnown; 起始索引为-1；
        ...
        {
            if (_length > 0) //have unlocked match 如果匹配（两个行相同）
            {
